...
def upgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE memory_entries ALTER COLUMN confidence TYPE text USING (CASE WHEN confidence > 0.66 THEN 'HIGH' WHEN confidence > 0.33 THEN 'MEDIUM' ELSE 'LOW' END)"
    )
...
def downgrade() -> None:
    ...
    op.execute(
        "ALTER TABLE memory_entries ALTER COLUMN confidence TYPE double precision USING (CASE WHEN confidence = 'HIGH' THEN 1.0 WHEN confidence = 'MEDIUM' THEN 0.5 ELSE 0.0 END)"
    )
