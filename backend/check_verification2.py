...
async def main():
    async with async_session_factory() as session:
        ...
        res = await session.execute(
            select(Conversation).order_by(Conversation.created_at.desc()).limit(15)
        )
        ...
        res = await session.execute(
            select(ActionLog).where(ActionLog.action_type == 'search_document').order_by(ActionLog.created_at.desc()).limit(5)
        )
