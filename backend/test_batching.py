...
from app.db.models import ActionLog, EmailMessage, User
...
async def main():
    async with async_session_factory() as session:
        ...
        e1 = EmailMessage(
            ...
            snippet="Hi, can you send the report?",
            needs_reply=True,
            received_at=now,
            subject="Report",
        )
        ...
        response = await handle_message(
            session, user, "I have some free time. What should I knock out? Do I have any batches pending?"
        )
        ...
        res = await session.execute(
            select(ActionLog)
            .where(ActionLog.user_id == user.id, ActionLog.action_type == "draft_email_reply")
