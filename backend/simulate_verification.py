...
async def main():
    async with async_session_factory() as session:
        ...
        doc_id = uuid4()
        doc = Document(id=doc_id, user_id=user.id, filename="Project_Orion_Spec.txt", source="upload", full_text=full_text, summary="A test document summary.")
        session.add(doc)
        await session.flush()
        ...
        for idx, chunk_text_str in enumerate(chunks):
            ...
            chunk = DocumentChunk(document_id=doc.id, user_id=user.id, chunk_text=chunk_text_str, chunk_index=idx, embedding=embedding)
            session.add(chunk)
