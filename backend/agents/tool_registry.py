...
create_task_schema = FunctionDeclaration(
    name="create_task",
    description="Create a new task for the user. If contact_name is provided, it tries to link the task to an existing contact.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "title": {"type": "STRING", "description": "Task title"},
            "due_at": {"type": "STRING", "description": "ISO 8601 datetime string for due date (optional)"},
            "priority": {"type": "STRING", "description": "Task priority (optional)"},
            "project": {"type": "STRING", "description": "Project name (optional)"},
            "contact_name": {"type": "STRING", "description": "Name of contact to link (optional)"},
        },
        "required": ["title"],
    },
)

create_reminder_schema = FunctionDeclaration(
    name="create_reminder",
    description="Set a reminder for the user.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "type": {"type": "STRING", "description": "One of: time, date, recurring, event, context, location"},
            "trigger_payload": {"type": "OBJECT", "description": "JSON payload containing reminder trigger specifics (e.g. {'datetime': '2023-10-27T18:00:00Z'} for time)"},
        },
        "required": ["type", "trigger_payload"],
    },
)

create_calendar_event_schema = FunctionDeclaration(
...
search_memory_schema = FunctionDeclaration(
    name="search_memory",
    description="Search the user's memory for relevant facts, preferences, people, dates, or context.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "query": {"type": "STRING", "description": "The search query to match against memories"},
            "category": {"type": "STRING", "description": "Optional category filter (person, preference, date, promise, context)"},
        },
        "required": ["query"],
    },
)

store_memory_schema = FunctionDeclaration(
    name="store_memory",
    description="Save a new memory or fact about the user. Importance score is generated by you later.",
    parameters={
        "type": "OBJECT",
        "properties": {
            "content": {"type": "STRING", "description": "Memory content/details"},
            "category": {"type": "STRING", "description": "Must be one of: person, preference, date, promise, context"},
            "importance_score": {"type": "NUMBER", "description": "Score from 0.0 to 1.0 (optional)"},
        },
        "required": ["content", "category"],
    },
)

find_contact_schema = FunctionDeclaration(
    name="find_contact",
    description="Find a contact by fuzzy name matching.",
    parameters={
        "type": "OBJECT",
        "properties": {
