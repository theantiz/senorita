...
export function AppWrapper({ children }: { children: React.ReactNode }) {
  ...
  const handleSetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await setupAuth(nameInput.trim(), timezone);
      if (res.token) {
        setToken(res.token);
      }
    } catch (err: any) {
            ...
            <div className="bg-[rgba(255,255,255,0.02)] border border-[rgba(255,255,255,0.09)] backdrop-blur-md p-8">
              ...
              {activeTab === "login" ? (
                <form onSubmit={handleLogin} className="space-y-6">
                  ...
                  <button
                    type="submit"
                    className="w-full bg-white text-black font-display font-medium text-sm px-4 py-3 hover:bg-white/90 transition-colors tracking-wide"
                  >
                    LOGIN TO SYSTEM
                  </button>
                </form>
